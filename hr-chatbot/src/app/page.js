...
export default function InterviewPage() {
  ...
  const sendMessage = async (overrideMessage) => {
    ...
    try {
      ...
      // Handle role questions state
      if (data.roleQuestions) {
        setRoleQuestions(data.roleQuestions)
        setRoleQIndex(data.roleQIndex ?? 0)
      }
      if (data.nextStep === 'role_specific_questions' && typeof data.roleQIndex === 'number') {
        setRoleQIndex(data.roleQIndex)
      }
      ...
      setTimeout(() => {
        ...
        // Show widgets after message
        setTimeout(() => {
          if (data.showResumeUpload) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0f172a] via-[#1e1b4b] to-[#0f172a] flex items-center justify-center px-4">
      <div className="text-center max-w-lg">
        ...
        <p className="text-slate-500 text-sm mb-10">Answer honestly. Our system is fair, structured and judgment-free.</p>
        ...
        <button
          onClick={onStart}
          className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#4361ee] to-[#7b5ea7] text-white font-semibold text-lg hover:opacity-90 transition-all shadow-xl"
        >
          Start Interview →
        </button>
