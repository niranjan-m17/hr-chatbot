...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (authChecked) fetchCandidates()
  }, [authChecked, fetchCandidates])

  const handleShortlist = async () => {
    if (roleFilter === 'all') {
      alert('Please select a specific role first, then click Auto-Shortlist')
      return
    }
    ...
    if (data.success) {
      alert(`✅ Shortlisted top ${data.count} candidates for ${roleFilter}`)
      fetchCandidates()
    }
  }
  ...
  const handleStatusChange = async (id, status) => {
    await fetch('/api/candidates', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id, status, is_shortlisted: status === 'shortlisted' }),
    })
    fetchCandidates()
    if (selectedCandidate?.id === id) {
      setSelectedCandidate(prev => ({ ...prev, status, is_shortlisted: status === 'shortlisted' }))
    }
  }
      ...
      <nav className="bg-white border-b border-slate-100 sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleLogout} className="text-xs text-slate-500 hover:text-slate-800 transition-colors px-3 py-1.5 rounded-lg hover:bg-slate-100">
              Sign out
            </button>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'Interviews Done', value: stats.completed, color: 'text-blue-600', bg: 'bg-blue-50' },
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          <div className="p-5 border-b border-slate-50 flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2 bg-[#4361ee] text-white rounded-xl text-sm font-semibold hover:bg-[#3451d1] disabled:opacity-50 transition-colors whitespace-nowrap"
            >
          ...
          {/* Table */}
          {loading ? (
            <div className="py-20 text-center text-slate-400 text-sm">Loading candidates...</div>
          ) : filtered.length === 0 ? (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-50 bg-slate-50/50">
                    {['Rank', 'Candidate', 'Role Applied', 'Experience', 'Score', 'Typing', 'Status', 'Actions'].map(h => (
                ...
                <tbody>
                  {filtered.map((c, i) => {
                    const score = Math.round(c.hr_score_override || c.total_score || 0)
                    const badge = getScoreBadge(score)
                    return (
                      <tr
                        key={c.id}
                        onClick={() => setSelectedCandidate(c)}
                        className={`border-b border-slate-50 hover:bg-slate-50/50 transition-colors cursor-pointer ${c.is_shortlisted ? 'bg-emerald-50/20' : ''}`}
                      >
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-lg bg-[#4361ee]/10 flex items-center justify-center flex-shrink-0">
                              <span className="text-[#4361ee] text-xs font-bold">{c.full_name?.charAt(0)?.toUpperCase() || '?'}</span>
                            </div>
                            <div>
                              <div className="text-sm font-semibold text-slate-800 flex items-center gap-1">
                                {c.full_name || 'In Progress'}
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-2">
                            ...
                            {score > 0 && (
                              <span className={`text-xs px-1.5 py-0.5 rounded-md font-medium ${badge.color}`}>{badge.label}</span>
                            )}
                          </div>
                        </td>
                        <td className="px-4 py-3.5 text-xs text-slate-600">
                        ...
                        <td className="px-4 py-3.5" onClick={e => e.stopPropagation()}>
                          <button
                            onClick={() => setSelectedCandidate(c)}
                            className="text-xs text-[#4361ee] hover:underline font-medium"
                          >
                            View Profile
                          </button>
                        </td>
