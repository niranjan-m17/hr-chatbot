...
export default function InterviewPage() {
  ...
  const sendMessage = async (overrideMessage) => {
    ...
    try {
      ...
      // Show bot response with delay
      setTimeout(() => {
        addMessage('assistant', data.botResponse)
        setTimeout(() => {
          if (data.showResumeUpload && !resumeUploaded) {
            setShowResumeUpload(true)
            setShowTypingTest(false)
          }
          if (data.showTypingTest) {
  ...
  const inputDisabled = isLoading || showTypingTest || (showResumeUpload && !resumeUploaded) || isComplete
    ...
    <div className="flex flex-col h-screen" style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #eff6ff 100%)' }}>
      <header className="bg-white border-b border-slate-100 shadow-sm flex-shrink-0">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-semibold text-slate-900">Zara AI Assistant</h1>
            <p className="text-xs text-slate-500">Powered by TalentScreen</p>
          </div>
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
          ...
          {/* Resume Upload Widget - stays visible until uploaded */}
          {showResumeUpload && !resumeUploaded && candidateId && (
          ...
          <div className="flex gap-2 items-end">
            <textarea
              ...
              placeholder={
                ...
                (showResumeUpload && !resumeUploaded) ? '📎 Please upload your resume using the widget above...' :
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'linear-gradient(135deg, #0f172a, #1e1b4b)' }}>
      <div className="text-center max-w-lg">
        ...
        <h1 className="text-4xl font-bold text-white mb-3">Meet Zara, Your Interview AI</h1>
        <p className="text-slate-400 text-lg mb-2">Fair · Smart · Takes only 15–20 minutes</p>
        <p className="text-slate-500 text-sm mb-10">A friendly AI-powered interview experience designed for your success</p>
        <div className="grid grid-cols-3 gap-3 mb-10">
          {[{ icon: '💬', label: 'Conversational' }, { icon: '📄', label: 'Resume Check' }, { icon: '⌨️', label: 'Skills Test' }].map(item => (
        ...
        <button onClick={onStart} className="w-full py-4 rounded-2xl text-white font-semibold text-lg transition-all shadow-xl hover:opacity-90"
          ...
          Start Interview with Zara →
        </button>
        <p className="text-slate-600 text-xs mt-4">Your data is secure · Reviewed only by authorized HR personnel</p>
      </div>
    </div>
  )
}
